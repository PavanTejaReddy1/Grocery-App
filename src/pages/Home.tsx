...
export default function HomeScreen() {
    ...
    const handleSelectResult = (id: string | number) => {
        setSearchQuery("");
        navigate(`/product/${id}`);
    };
            ...
            <div className="md:hidden h-screen overflow-hidden flex items-center justify-center px-4">
                <div className="relative w-full max-w-[400px] h-[680px] bg-white rounded-2xl shadow-xl overflow-y-auto scrollbar-hide">
                    ...
                    <div className="px-4 mt-6 relative">
                        ...
                        {normalizedQuery && (
                            <div className="absolute left-4 right-4 top-[58px] z-30 bg-white rounded-[15px] shadow-lg border border-[#E2E2E2] max-h-[320px] overflow-y-auto">
                                {filteredProducts.length > 0 ? (
                                    filteredProducts.map((item) => (
                                        <div
                                            key={item.id}
                                            onClick={() => handleSelectResult(item.id)}
                                            className="flex items-center gap-3 px-4 py-3 hover:bg-[#F2F3F2] cursor-pointer border-b border-[#F2F3F2] last:border-b-0"
                                        >
                                            <img src={item.image} alt={item.name} className="w-10 h-10 object-contain" />
                                            <div className="flex-1 min-w-0">
                                                <p className="text-[14px] font-semibold text-[#181725] truncate">{item.name}</p>
                                    ...
                                    <div className="px-4 py-5 text-center text-[14px] text-[#7C7C7C]">
            ...
            <div className="hidden md:flex flex-col min-h-screen">
                ...
                <header className="sticky top-0 z-20 bg-white shadow-sm">
                    <div className="max-w-7xl mx-auto px-8 h-[70px] flex items-center justify-between">
                        ...
                        {/* Search */}
                        <div className="relative flex-1 mx-8 max-w-[400px]">
                            <div className="h-[44px] bg-[#F2F3F2] rounded-[15px] flex items-center px-4">
                            ...
                            {/* Search results dropdown */}
                            {normalizedQuery && (
                                <div className="absolute left-0 right-0 top-[50px] z-30 bg-white rounded-[15px] shadow-lg border border-[#E2E2E2] max-h-[360px] overflow-y-auto">
                                    {filteredProducts.length > 0 ? (
                                        filteredProducts.map((item) => (
                                            <div
                                                key={item.id}
                        ...
                        {normalizedQuery ? (
                            ...
                            <div>
                                <div className="flex items-center justify-between mb-4">
                                    ...
                                    <button
                                        onClick={clearSearch}
                                        className="text-[14px] font-semibold text-[#53B175] cursor-pointer"
                                    >
                                        Clear search
                                    </button>
                                </div>
                                ...
                                {filteredProducts.length > 0 ? (
                                    <div className="grid grid-cols-4 gap-4">
                                        {filteredProducts.map((item) => (
                                            <div key={item.id} onClick={() => handleSelectResult(item.id)}
                                                ...
                                                <div className="flex items-center justify-between w-full mt-3">
                                                    <span className="text-[16px] font-bold text-[#181725]">${item.price}</span>
                                                    <button className="w-[36px] h-[36px] bg-[#53B175] rounded-full flex items-center justify-center text-white text-[24px] leading-none">+</button>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                                ) : (
                                    <div className="bg-white border border-[#E2E2E2] rounded-[18px] py-16 text-center text-[#7C7C7C]">
